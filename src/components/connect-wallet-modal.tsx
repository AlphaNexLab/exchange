...
function hintFor(connector: Connector): string {
  const key = connector.name.toLowerCase();
  for (const [name, hint] of Object.entries(WALLET_HINTS)) {
    if (key.includes(name)) return hint;
  }
  if (connector.type === "walletConnect") return "QR / mobile wallets";
  ...
  if (connector.icon) {
    return (
      ...
      <img
        src={connector.icon}
        alt=""
        className="w-8 h-8 rounded-lg object-contain bg-space-900/60"
      />
    );
  }
...
export function ConnectWalletModal() {
  ...
  const listed = useMemo(() => {
    ...
    const generic = connectors.filter(
      (c) => c.type === "injected" && c.name.toLowerCase() === "injected"
    );
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-center justify-center p-4">
      ...
      <div
        ...
        className="relative w-full max-w-md rounded-2xl border border-slate-700/50 bg-space-800/95 shadow-2xl"
      >
        <div className="flex items-start justify-between gap-4 p-5 border-b border-slate-700/40">
          ...
          <button
            type="button"
            onClick={closeModal}
            className="p-2 rounded-lg text-slate-400 hover:text-white hover:bg-space-700/60 transition-colors"
            aria-label="Close"
          >
            <X className="w-4 h-4" />
          </button>
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    "border-slate-700/40 bg-space-900/40 hover:border-amber-500/40 hover:bg-amber-500/5",
        ...
        <div className="p-4 pt-2 border-t border-slate-700/40 space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            >
              Get Rabby <ExternalLink className="w-3 h-3 ml-1" />
            </Button>
            <Button
              variant="outline"
              size="sm"
